...
class LibDatagenItemModel(
	...
	private fun extendWithFolder(rl: ResourceLocation): ResourceLocation {
		...
		else ResourceLocation.fromNamespaceAndPath(
			rl.namespace,
			"${folder}/${rl.path}"
		)
	}
	...
	private fun withExistingParent(registry: DeferredRegister.Items, pathPrefix: String) {
		registry.entries.stream()
			.map { it.id }
			.forEach { itemId: ResourceLocation ->
	...
	fun createModelFile(
		item: Item,
		prefix: String,
		textures: MutableMap<Float, String>,
		parent: ModelFile?,
		...
		for ((index, entry) in textures.entries.withIndex()) {
			...
			modelBuilder.override()
				...
				.predicate(predicates[min(index, predicates.size - 1)], entry.key)
				.end()
	...
	fun customModelFile(name: String): ModelFile {
		return ModelFile.UncheckedModelFile(modRl(name))
	}
}
